...
class Target(lg.LocalTarget):
    ...
    def __init__(self, task, path):
        ...
        self.pathMeta = str(Path(ConfigData().pathMeta).resolve() / "{}.{}.json".format(pathForMeta, task.getShortStamp()))
    ...
    def computeSrcMeta(self):
        return {
            'task': self.task.getSignature(),
            'code': self.task.getCodeHash(),
            'src': self.task.getSrcHash(),
            }

    def computeGenMeta(self):
        objGen = self.computeSrcMeta()
        if self.task.checkOutputHash:
            pathThis = Path(self.path)
            if pathThis.is_dir():
                h = md5(usedforsecurity=False)
                for f in sorted((p for p in pathThis.glob('**/*') if p.is_file())):
                    if self.task.simplifiedOutputHash:
                        h.update(bytes(f.name, 'utf-8'))
                    else:
                        h.update(f.read_bytes())
                objGen['out'] = h.hexdigest()
            else:
                objGen['out'] = md5(pathThis.read_bytes(), usedforsecurity=False).hexdigest()
        else:
            objGen['out'] = '0'
    ...
    def writeMeta(self):
        ...
        objMeta = {'data': {}, 'gen': self.computeGenMeta()}
    ...
    def isOutdated(self, reCompute=False):
        ...
        if reCompute:
            if objGen != self.computeGenMeta():
                return True

        return False

class WrapperTarget(Target):
    def __init__(self, task, tgt):
        super().__init__(task, tgt.path)
